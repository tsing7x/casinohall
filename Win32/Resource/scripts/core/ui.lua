...
ScrollView.s_ContTotalOffset = 0
---
-- 默认的滚动条宽度.
-- 
...
ScrollView.resetScrollContentSize = function(self, width, height)
    -- body
    if self.m_autoPositionChildren then
        --todo
        self.m_nodeW = width == 0 and self.m_nodeW or width
        self.m_nodeH = height == 0 and self.m_nodeH or height
    end
...
ScrollView.getContScrollOffset = function(self)
    -- body
    return self.s_ContTotalOffset
end
...
ScrollView.onScroll = function(self, scroll_status, diffY, totalOffset,isMarginRebounding)
    ...
    self.s_ContTotalOffset = totalOffset
    if self.m_direction == kVertical then
...
Switch.reviseSize = function(self)
    Node.reviseSize(self)
    Node.setClip2(self, true, 0, 0, self.m_width,self.m_height);
end
...
Text.setColor = function(self, r, g, b)
    if self.m_res then
        self.m_res.m_r = r
        self.m_res.m_g = g
        self.m_res.m_b = b
    end
    DrawingImage.setColor(self, r, g, b)
end
...
-- 设置当前文字的对齐方式。
-- 
-- @param #number 当前文字对齐方式。
Text.setTextAlign = function(self , align)
    self.m_res:setTextAlign(align);
    Text.addImage(self,self.m_res,0); 
    Text.setSize(self,self.m_res:getWidth(),self.m_res:getHeight());
end
...
-- 
-- @param #number 当前文字对齐方式。
TextView.setTextAlign = function(self , align)
    local w,h = TextView.getSize(self);
    ...
    local drawingW,drawingH = self.m_drawing:getSize();
    TextView.setSize(self,width,drawingH);
    local x,y = TextView.getUnalignPos(self);
    local w,h = TextView.getSize(self);
    TextView.setClip(self,x,y,w,h);
