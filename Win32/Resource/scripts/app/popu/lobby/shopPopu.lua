...
function ShopPopu:initRecordItem(record)
	...
	if record.status == 2 then
		item:findChildByName("status"):setFile("popu/shop/record/2.png")
	elseif record.status == 1 then
		item:findChildByName("status"):setFile("popu/shop/record/1.png")
	else
		item:findChildByName("status"):setFile("popu/shop/record/3.png")
	end
	return item
end
