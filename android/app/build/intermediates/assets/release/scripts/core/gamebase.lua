...
--virtual
GameSocket.onReceivePacket = function(self, cmd, info,gameid)
	if cmd~=0x600d then
		writeTabToLog({info=info,cmd=string.format("0x%02x",cmd),gameid=gameid}, "收到socket包","debug_socket.lua")
	end
	for k,v in pairs(self.m_socketProcessers) do
		local info =  v:onReceivePacket(cmd,info,gameid);
		if info then
			return info;
		end
	end
	...
	for k,v in pairs(self.m_commonSocketProcessers) do
		...
		if info then
			for k,v in pairs(self.m_socketProcessers) do
				if v:onCommonCmd(cmd,cmdInfo,gameid) then
					break;
				end
			end
			return;
...
GameSocket.parseMsg = function(self, packetId)
	local cmd,gameid = self:readBegin(packetId);
	local info = self:readPacket(self.m_socket,packetId,cmd);
	self:readEnd(packetId);
	return cmd,info,gameid;
end

GameSocket.onSocketServerPacket = function(self, packetId)
	GameSocket.stopTimer(self);
	local cmd,info,gameid = GameSocket.parseMsg(self,packetId);
	self:onReceivePacket(cmd,info,gameid);
end
...
HttpManager.checkCommand = function(self, command)
	...
	repeat 
		if not (command and self.m_configMap[command]) then
...
HttpManager.onResponse = function(self , httpRequest)
	...
	repeat 
		...
		print_string("zyh requestUrl ===========>> " .. httpRequest:getRequestUrl());
        print_string("zyh resultStr  ===========>> " .. resultStr);
...
SocketProcesser.onReceivePacket = function(self,cmd,packetInfo,gameid)
    if self.s_severCmdEventFuncMap[cmd] then
        local info = self.s_severCmdEventFuncMap[cmd](self,packetInfo,gameid);
        return info or {};
    end
    return nil;
end
...
SocketReader.ctor = function(self)
	self.m_socket = nil;	-- æ“ä½œçš„å¥—æŽ¥å­—
end
...
-- @return #string 
-- @usage local ver = require 'gameBase.version' -- verä¸ºä¸€ä¸ªå­—ç¬¦ä¸²ï¼Œå†…å®¹æ˜¯libgamebaseçš„ç‰ˆæœ¬å·ã€?
