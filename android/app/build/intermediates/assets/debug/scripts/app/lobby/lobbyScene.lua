...
function LobbyScene:onMessageBtnClick()
	printInfo("LobbyScene:onMessageBtnClick called!")

	-- NativeEvent.getInstance():actCenterDisplay()
	NativeEvent.getInstance():actCenterDisplay()
	do return end
...
function LobbyScene:onBtnHeadClick()
	-- printInfo("LobbyScene:onBtnHeadClick called!")

	NativeEvent.getInstance():callActCenter({languageId = 1})
	do return end
...
function LobbyScene:onNativeCallBack(key, data, result)
	...
	if key == kActCenterCall then
	    -- self:onAcycenterJump(data)
...
-- 处理活动界面事件
function LobbyScene:onAcycenterJump(data)
