...
NativeEvent.callActCenter = function(self, param)
	-- body
	local debugFlag = param.debug or 1
	local languageId = param.languageId

	local siteMid = string.format(PhpManager:getDevice_id() .. "_gamehallboyaa")

	local activityParam = {
		key = "boyaa@@*9010",
		-- key = ACTIVITY_SECRET_KEY,
		url = "http://shisanzhang.ifere.com",
		-- url = debugFlag > 0 and "http://actcenter.ifere.com/operating/web/index.php" or "http://mvlptldc.boyaagame.com",
		mid = MyUserData:getId(),
		api = PhpManager:getGame(),
		channelId = "",
		sitemid = siteMid,
		...
		deviceNo = siteMid,
		gameId = "9010",
		-- gameId = ACTIVITY_APPID,
		languageId = languageId,
		debug = debugFlag
	}

	self:callNativeEvent(kActCenter, json.encode(activityParam))
end

NativeEvent.actCenterSwitchSvr = function(self, param)
	-- body
	local paramData = {
		serverId = param or 1
	}

	self:callNativeEvent(kActcenterSwicthSvr, json.encode(paramData))
end

NativeEvent.actCenterSetSkin = function(self, param)
	-- body
	local paramData = {
		skinVersion = param or "V1"
	}

	self:callNativeEvent(kActcenterSetSkin, json.encode(paramData))
end

NativeEvent.actCenterDisplay = function(self)
	-- body
	self:callNativeEvent(kActcenterDisplay)
end

NativeEvent.actCenterDisplayAct = function(self, param)
	-- body
	local paramData = {
		actId = param or ""
	}

	self:callNativeEvent(kActcenterDisplayAct, json.encode(paramData))
end

NativeEvent.actCenterDisplayRelate = function(self, param)
	-- body
	local paramData = {
		size = param or 1
	}

	self:callNativeEvent(kActcenterDisplayRelate, json.encode(paramData))
end

NativeEvent.actCenterClearCache = function(self)
	-- body
	self:callNativeEvent(kActcenterClearCache)
end
